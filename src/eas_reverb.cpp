...
S_REVERB_OBJECT *ReverbInit()
{
...
#else
    pReverbData = (S_REVERB_OBJECT*) malloc(sizeof(S_REVERB_OBJECT));
    //pReverbData = new S_REVERB_OBJECT;
#endif
    ...
    /* clear the structure */
    memset(pReverbData, 0, sizeof(S_REVERB_OBJECT));
    ...
    *pInstData = pReverbData;
    ...
    fprintf(stderr, "reverb object addr in ReverbInit: 0x%x\n", pReverbData);
...
void ReverbProcess(S_REVERB_OBJECT *pReverbData, EAS_PCM *pSrc, EAS_PCM *pDst, EAS_I32 numSamples)
{
    ...
    {
        if (pSrc != pDst)
            memcpy(pDst, pSrc, numSamples * NUM_OUTPUT_CHANNELS * (EAS_I32) sizeof(EAS_PCM));   // Modified to fit with DPF (swap pSrc and pDst, which is diifferent from EAS)
        return;
    }
 ...
 *----------------------------------------------------------------------------
*/
#if 0   // This DOES NOT work in C++!
EAS_RESULT ReverbShutdown (S_REVERB_OBJECT *pReverbData)
{
    free(pReverbData);
